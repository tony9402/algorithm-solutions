...
// Co-authored by: -
// Link: https://leetcode.com/problems/greatest-common-divisor-traversal/submissions/1185765234?envType=daily-question&envId=2024-02-25
class Solution {
public:
    bool canTraverseAllPairs(vector<int>& nums) {
        const int M = *max_element(nums.begin(), nums.end()) + 1;
        vector<int> P(M+1); iota(P.begin(), P.end(), 0);
        for(int i = 2; i * i <= M; ++i) {
            if(P[i] != i) continue;
            for(int j = i * i; j <= M; j += i) if(P[j] == j) P[j] = i;
        }
        int N = (int)nums.size();
        vector<int> uf(N), siz(N, 1); iota(uf.begin(), uf.end(), 0);
        function<int(int)> find = [&](int x) -> int { while(uf[x] != x) uf[x] = uf[uf[x]], x = uf[x]; return x; };
        function<void(int, int)> merge = [&](int u, int v) -> void {
            u = find(u); v = find(v);
            if(u == v) return;
            uf[v] = u; siz[u] += siz[v]; siz[v] = 0;
        };
        vector<int> used(M + 1, -1);
        for(int i = 0; i < N; ++i) {
            int pre = -1;
            for(int j = nums[i]; j != 1; j /= P[j]) {
                if(pre == P[j]) continue;
                pre = P[j];
                if(used[P[j]] != -1) merge(used[P[j]], i);
                else used[P[j]] = i;
            }
        }
        return siz[find(0)] == N;
